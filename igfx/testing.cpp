...
ID                           :#2105061
SUPERVISOR                   :Preetom Saha Arko sir
last version                 :4.0(Stable)
date_last_modified           :3-1-2023,Wednesday,12:19 AM
Copyright © Ibtida Bin Ahmed,2023-
All Rights Reserved
*/
#include"iGraphics.h"
#include<stdio.h>
...
void changes()
{
    static int xyz=1;
    srand(time(NULL)%60);
    int rnd=pow(-1,(xyz*1.0))*rand()+200;
    ...
    for (int i=0; i<50; i++)
    {
        ...
        {
            stars[i].stars_y=screenheight+(rnd+100)%(screenheight/2);
        }
        stars[i].stars_index++;
    ...
    for (int i=0; i<ENEMYCT; i++)
    {
        ...
        if(tango[i].spawn_loc_y<=0)
        {
            ...
            tango[i].spawn_loc_x=spawn_loc_xs[i]+pow(-1,(i*1.0))*rnd;
            ++tango[i].tango_index;
        }
...
int main()
{
    ...
    {
        int rnd=rand();
        spawn_loc_xs[i]=140+rnd%(screenwidth/2);
        //printf("%d\n",spawn_loc_xs[i]);
    }
    len = 0;
    mode = 2;
    ...
    iInitialize(screenwidth, screenheight, "space shooters");
